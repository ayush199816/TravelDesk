...
const QuoteBuilder = ({ lead, quote, onClose, onSave }) => {
  ...
  useEffect(() => {
    const loadExchangeRates = async () => {
      try {
        ...
        console.log('📊 Real-time exchange rates loaded in QuoteBuilder');
      } catch (error) {
  ...
  const fetchAvailableServices = useCallback(async () => {
    try {
      ...
      console.log('Setting available hotels:', hotelsResponse.data);
      console.log('First hotel roomCategories:', hotelsResponse.data[0]?.roomCategories);
      ...
      try {
        ...
        console.error('Fallback hotel fetch also failed:', hotelError);
      }
    } finally {
      setLoading(false);
    }
  ...
  const fetchHotels = useCallback(async () => {
    try {
      ...
      console.error('Error fetching hotels independently:', error);
    }
  ...
  useEffect(() => {
    if (user.organization._id) {
      console.log('Organization available, fetching hotels');
      fetchHotels();
    }
  ...
  useEffect(() => {
    console.log('useEffect triggered for fetching services');
    console.log('quoteData.country:', quoteData.country);
    console.log('quoteData.travelStartDate:', quoteData.travelStartDate);
    console.log('quoteData.travelEndDate:', quoteData.travelEndDate);
    console.log('user.organization._id:', user.organization._id);
    
    if (quoteData.country && quoteData.travelStartDate && quoteData.travelEndDate && user.organization._id) {
      console.log('All conditions met, calling fetchAvailableServices');
      fetchAvailableServices();
    } else {
      console.log('Conditions not met for fetching services');
    }
      ...
      if (quote.country && quote.travelStartDate && quote.travelEndDate) {
        const fetchServicesForQuote = async () => {
          try {
            ...
            console.error('Error fetching services for quote:', error);
          } finally {
            setLoading(false);
          }
        };
  ...
  const addRoomToHotel = (hotelIndex, roomCategory) => {
    console.log('🏨 DEBUG: Adding room to hotel:', {
      hotelIndex,
      roomCategory: roomCategory.name,
      roomCategoryId: roomCategory._id,
      currentRooms: quoteData.hotels[hotelIndex]?.rooms?.length || 0
    });
    ...
    setQuoteData(prev => {
      ...
      console.log('🏨 DEBUG: Pushing new room:', newRoom);
      console.log('🏨 DEBUG: Before push - rooms count:', newHotels[hotelIndex].rooms.length);
      
      newHotels[hotelIndex].rooms.push(newRoom);
      
      console.log('🏨 DEBUG: After push - rooms count:', newHotels[hotelIndex].rooms.length);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Debug: Log the payload to verify no Promise objects
      console.log('🔍 DEBUG: Quote payload being sent:', {
        quoteNumber: quotePayload.quoteNumber,
        currency: quotePayload.currency,
        days: quotePayload.days?.map((day, dayIndex) => ({
          dayIndex,
          sightseeingsCount: day.sightseeings?.length || 0,
          sightseeings: day.sightseeings?.map((s, i) => ({
            index: i,
            name: s.sightseeing,
            adultRate: s.adultRate,
            adultRateType: typeof s.adultRate,
            childRate: s.childRate,
            childRateType: typeof s.childRate,
            rate: s.rate,
            rateType: typeof s.rate,
            currency: s.currency
          })),
          transfersCount: day.transfers?.length || 0,
          transfers: day.transfers?.map((t, i) => ({
            index: i,
            name: t.name,
            rate: t.rate,
            rateType: typeof t.rate,
            currency: t.currency
          }))
        })),
        hotels: quotePayload.hotels?.map((h, i) => ({
          index: i,
          name: h.name,
          roomsCount: h.rooms?.length || 0,
          rooms: h.rooms?.map((r, j) => ({
            roomIndex: j,
            roomName: r.roomName,
            adultRate: r.adultRate,
            adultRateType: typeof r.adultRate,
            currency: r.currency
          }))
        }))
      });
                  ...
                  >
                    ...
                    {(() => {
                      if (hotelItem.isTemporary) {
                        ...
                        console.log('Looking for hotel with ID:', hotelItem.hotel, 'found:', foundHotel);
                        console.log('Available hotels:', availableHotels);
                        console.log('Room categories for found hotel:', foundHotel?.roomCategories);
